...
def main():
  ...
  folder_path = fd.askdirectory(parent=root,initialdir="/Users/florianfurrer/OneDrive - PHZH/Dokumente/NAWs für Auswertungsprogramm/",title='Wo liegen die NAW-Tests? (Unterordner erlaubt)') #change to "folder_path"

  root.title("NAW-CSV Datei, um Werte anzuhängen")
  csv_path = fd.askopenfilename(parent=root,initialdir="/Users/florianfurrer/OneDrive - PHZH/Dokumente/NAWs für Auswertungsprogramm/",title='An welches CSV die Werte anhängen?') #save filename of CSV
  ...
  for filename in all_files:
    ...
    a = re.search("[A-Z][0-9]{3}[_][A-Za-zäöüßÄÖÜ]{2}[0-9]{4}[_][0-9]{1}",filename) #Programm hat trotzdem Probleme mit Umlaut (returns Empty-Object in next step)
    naw_choice = a[0]
