...
import os
import glob, sys, fitz ## wird alles fuer PyMuPDF benoetigt
...
root.title("NAW-CSV Datei, um Werte anzuhaengen")
csv_path = fd.askopenfilename(parent=root,initialdir="/Users/florianfurrer/Documents/GitHub/naw-omr/",title='An welches CSV die Werte anhaengen?') #save filename of CSV
...
for wrange in range(0,20,1):
  for hrange in range(0,20,1):
    wr = wrange/10
    hr = hrange/10
    cfg.wh_ratio_range = (wr, hr) #(0.8, 1.2)
    chbox = 0
          ...
          for count2,i in enumerate(eval(coord_page),start=1): # eval() is used to select variable based on string
            ...
            if cb_xmin > i[0]-safety_border and cb_xmax < i[0]+safety_border and cb_ymin > i[1]-safety_border and cb_ymax < i[1]+safety_border:
              ...
              if checkbox[1] == True:
                ...
                ##MAKE HERE DATA-FRAME EINTRAG FUER APPEND IN CSV
    ...
    new_data = pd.DataFrame({'width-range': [wr],
                            'height-range': [hr],
                            'cb-total': [chbox]})
