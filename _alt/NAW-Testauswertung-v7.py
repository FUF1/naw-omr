...
keys = [
  "Orig_Filename","StudentIn","SuS","NAW-Version","exec-time","path",
...
def main():
  ...
  for filename in all_files:
    if "-marked" in filename:
      print("{} was already processed.".format(filename))
      break
  ...
  #  shortname = a.group(0)+b.group(0)
    ...
    if doc.page_count == 19:
      ...
      print("ERROR: Falschen Seitenanzahl bei {}. {} statt 19 Seiten.".format(shortname, doc.page_count))
      sys.exit() # abort program if wrong number of pages
  ...
  # Boxdetect
      ...
      ##maybe overkill - MAKE CHECK IF CHECKBOX TWICE ON SAME COORDINATE AND IF A COORDINATE WITHOUT CHECKBOX EXISTS
        ...
        for checkbox in checkboxes:
          ...
          for count2,i in enumerate(eval(coord_page),start=1): # eval() is used to select variable based on string
            #UNNECESSARY? #TRY: CREATE ERROR HANDLER?
            ...
            if cb_xmin > i[0]-safety_border and cb_xmax < i[0]+safety_border and cb_ymin > i[1]-safety_border and cb_ymax < i[1]+safety_border:
              ...
              if checkbox[1] == True:
                ...
                ##MAKE HERE DATA-FRAME EINTRAG FÜR APPEND IN CSV (X)
                  ## --> based on value of i (=CB-Nr.); create a function(?)
              ...
              ## WRITE ERROR CODES TO IMG AS RED TEXT!
    ...
    properties_to_csv(cb_dict, shortname, naw_choice, filename, csv_path)
...
def properties_to_csv(cb_dict, shortname, naw_choice, filename, csv_path):
  now = datetime.now()
